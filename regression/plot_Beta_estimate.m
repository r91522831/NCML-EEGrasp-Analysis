...
for All_sub_i = selected_sub
    ...
    for k = 1:3
        ...
        for electrode_i = 1:nb_electrode
            ...
            suptitle([sub_id, ' ', electrodes_name{electrode_i}, ' ', All_stat_plot{k}, ' ', ' (ERSP=\beta_0\cdotIL+\beta_1\cdotTR+\beta_2\cdotPT+\beta_3\cdotIL\cdotErr+\beta_4\cdotTR\cdotErr+\beta_5\cdotPT\cdotErr+E)'])

            filename_fig = [sub_id, '_', electrodes_name{electrode_i}, '_', All_stat_plot{k}, '.fig'];
            savefig(h, fullfile(All_fig_path, filename_fig),'compact');
        end
    end
end