function fig = contextICinspectPlot(EEG, data, context, time, blockavg, blockstd, comp, plotvar)
...
hold on
for i_epb = 2:2:6
    h(i_epb) = shadedErrorBar(0.001 * time, filtmat_class( dt(1), cutoff, blockavg(:, :, i_epb)' ), blockstd(:, :, i_epb), {'LineStyle', linspec{i_epb}, 'Color', lincolor{i_epb}}, 1);
end
hold off
% % %     ylim([-dRange, dRange]);
xlim(0.001 * [time(1), time(end)]);
...
subplot(3, 2, 5)
ep_all = nepb + 1;
h(ep_all) = shadedErrorBar(0.001 * time, filtmat_class( dt(1), cutoff, blockavg(:, :, ep_all)' ), blockstd(:, :, ep_all), {'LineStyle', linspec{ep_all}, 'Color', lincolor{ep_all}}, 1);
hold on
for i_epb = 8:2:10
    h(i_epb) = shadedErrorBar(0.001 * time, filtmat_class( dt(1), cutoff, blockavg(:, :, i_epb)' ), blockstd(:, :, i_epb), {'LineStyle', linspec{i_epb}, 'Color', lincolor{i_epb}}, 1);
end
