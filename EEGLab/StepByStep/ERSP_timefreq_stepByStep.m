...
selected_sub = input('Which subject(s) to run time freauency analysis? ');
...
% % %     tf_data = tf_ersp;
    ...
    total_iter = nb_epoch * length(All_selected_elec);
    %%
    % time frequency analysis for each channel and each epoch
    tf_tlim = [tfEEG.times(1), tfEEG.times(end)]; % in ms
...
% % %         tmp_itc = tmp_ersp;
% % %         tmp_data = tmp_ersp;
        tmpEEG = squeeze(tfEEG.data(electrodes{i}, :, :));
        for j = 1:nb_epoch % 
            % time freq analysis without any baseline normalization or subtraction
            [tmp_ersp{j, 1}, tmp_freqs, tmp_times] = timefreq( tmpEEG(:, j), tfEEG.srate, ...
                                                               'cycles', [3, 0.5], ...
...
% % %             [tmp_ersp{j, 1}, tmp_itc{j, 1}, ~, tmp_times, tmp_freqs, ~, ~, tmp_data{j, 1}] = ...
            [tmp_ersp{j, 1}, ~, ~, tmp_times, tmp_freqs, ~, ~, ~] = ...
...
% % %     tf_data = cell2table(tf_data(~cellfun('isempty', tf_data)), 'RowNames', {EEG.chanlocs(All_selected_elec).labels});
    ...
    save(tmp_filename, 'tf_times', 'tf_freqs', 'tf_ersp', '-v7.3'); %, 'tf_*');
