...
for j = 1:length(session)
    ...
    for i = 1:length(file_list)
        ...
        if ~strcmp(file_list(i).name(:, 11:12), session{j})
            continue;
        end
    ...
    if strcmp(session{j}, 'PT')
        for k = 1:3
            timestamp_go2fini = timestamp_go2fini(k:3:end, 1);
            obj_go2fini = obj_go2fini(k:3:end, 1);
            th_go2fini = th_go2fini(k:3:end, 1);
            re_go2fini = re_go2fini(k:3:end, 1);
            in_go2fini = in_go2fini(k:3:end, 1);
            mi_go2fini = mi_go2fini(k:3:end, 1);
            ...
            ylabel('roll ({\circ})')
            xlabel('time (s)')
            mtit([file_list(1, 1).name(1:4), ' object pose in ', session{j}, num2str(k), ' ', obj_side])
            savefig(fullfile(pathname_plot, 'plots', [file_list(1, 1).name(1:4), ['_obj_pose_', session{j}, num2str(k)]]))
            ...
            ylabel('mz (N-mm)')
            xlabel('time (s)')
            mtit([file_list(1, 1).name(1:4), ' thumb in ', session{j}, num2str(k), ' ', obj_side])
            savefig(fullfile(pathname_plot, 'plots', [file_list(1, 1).name(1:4), ['_th_', session{j}, num2str(k)]]))
            ...
            ylabel('mz (N-mm)')
            xlabel('time (s)')
            mtit([file_list(1, 1).name(1:4), ' resultant in ', session{j}, num2str(k), ' ', obj_side])
            savefig(fullfile(pathname_plot, 'plots', [file_list(1, 1).name(1:4), ['_re_', session{j}, num2str(k)]]))
            ...
            ylabel('MI COPz (mm)')
            xlabel('time (s)')
            mtit([file_list(1, 1).name(1:4), ' fingers in ', session{j}, num2str(k), ' ', obj_side])
            savefig(fullfile(pathname_plot, 'plots', [file_list(1, 1).name(1:4), ['_fgr_', session{j}, num2str(k)]]))
        end
    else
        % for plot
        colorset = flipud(gray);%parula;%cool;%varycolor(19);
